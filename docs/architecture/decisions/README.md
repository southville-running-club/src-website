...
| [**ADR-002**](adr-002-schema-layout.md) | **Four schemas in one project**, and NN sign-ups land in `intake`. *Its `public` / `private` rows and its "who may migrate what" rule are superseded by [ADR-035](adr-035-the-timing-schema-joins-this-project.md); `club`, `intake` and the blast-radius argument stand* | 8 Aug 2026 |
...
| [**ADR-008**](adr-008-timing-port-before-the-race.md) | **The timing port lands before the race**, gated on a simulation with Vercel as the standing fallback. *Reverses the plan's earlier "after the race". **Superseded in full by [ADR-034](adr-034-the-timing-platform-is-rewritten-on-cloudflare.md)**, which makes the work a rewrite and switches Vercel off; the simulation gate and the three things that must not break are kept* | 9 Aug 2026 |
...
| [**ADR-032**](adr-032-an-email-is-sent-when-it-is-owed.md) | **An email is sent when it is owed, not when the cron next runs** - the drain moves to `ctx.waitUntil()` behind the Stripe webhook and every admin POST, so a confirmation arrives in seconds. *Amends [ADR-021](adr-021-the-club-tells-people-by-outbox.md), whose outbox, trigger and transaction are untouched; the five-minute cron stays as the retry net and because two of its three jobs are not email at all.* | 31 Aug 2026 |
| [**ADR-031**](adr-031-a-non-binary-entrant-says-where-to-be-placed.md) | **A non-binary entrant says where their result should be placed** — the nineteenth entry field, asking whether an existing result counts as women's, men's, or neither, resolved everywhere through one `effectiveCategory()` function so nothing downstream grows a third branch. *Extends [ADR-020](adr-020-race-category-and-gender-are-two-questions.md)'s open gap without closing it — a genuine third prize category is still the committee's decision, and this record says plainly it was not put to them.* | 31 Aug 2026 |
| [**ADR-033**](adr-033-a-ticket-is-not-an-entry.md) | **A ticket to a club social is not a race entry, and gets its own `store` schema** — `/events`, the Christmas party, Stripe and a confirmation email, kept clear of `entries` because `entries.entrants` requires a date of birth, a gender and an emergency contact that a party ticket has no business collecting. *Adds a schema, an `/events` section and a second Stripe webhook endpoint; supersedes nothing. The glossary's reserved word `event` is why the table is `socials`.* | 5 Sep 2026 |
| [**ADR-034**](adr-034-the-timing-platform-is-rewritten-on-cloudflare.md) | **The timing platform is rewritten on Cloudflare, and Vercel is switched off** — `src-race-timing` becomes the specification rather than the source, its tested pure functions are copied and everything else is written to this repository's conventions. *Supersedes [ADR-008](adr-008-timing-port-before-the-race.md) in full. The race-simulation gate and the three things that must not break are carried across unchanged; what the gate now decides is scope, because there is no second host to fall back to* | 10 Sep 2026 |
| [**ADR-035**](adr-035-the-timing-schema-joins-this-project.md) | **The timing schema is written here, as `timing`, and `public` stays empty** — new migrations in `packages/db` at current timestamps, not the old project's nineteen, because `db push` refuses a version earlier than the remote's newest by refusing the whole push. *Supersedes [ADR-002](adr-002-schema-layout.md)'s `public` / `private` rows and its two-migrator rule, both written on a premise — one project, two tenants — that was never true* | 10 Sep 2026 |
| [**ADR-036**](adr-036-timing-staff-are-identity-permissions.md) | **Timing staff are `identity` permissions, and the per-event roster is a grant** — `staff_assignments` is not written, `timing.marshals` says which events rather than who, and a marshal is deliberately not staff. *Extends [ADR-017](adr-017-permissions-are-what-code-checks.md) to the second application, and takes the seventh-role/twelfth-permission stop-and-ask on purpose* | 10 Sep 2026 |
| [**ADR-037**](adr-037-timing-stays-on-next-under-opennext.md) | **The timing app stays on Next.js under OpenNext, and does not adopt Tailwind** — a marshal screen, an offline queue and a live leaderboard are not pages with forms on them, which is what `apps/main`'s Astro shape is for. *Settles the framework, styling and test-layer questions ADR-034 opens, and names the one unmeasured thing that could overturn it: 3 MB compressed and 10 ms CPU* | 10 Sep 2026 |
| [**ADR-038**](adr-038-the-leaderboard-is-staff-only-in-2026.md) | **The live leaderboard is staff-only in 2026, and the results page is the only public surface** — behind `timing.event.manage` or `timing.crossing.resolve`, and the public sees nothing about a running race. *Supersedes the public-leaderboard assumption ADR-034's consequences and `phases.md` carried over from the old application's anonymous `/live/<slug>`. Records plainly that [C6](../../foundations/requirements.md#c6--show-live-race-progress-to-spectators) is **not met** in 2026 rather than quietly re-scoping it* | 12 Sep 2026 |
| [**ADR-039**](adr-039-the-roster-crosses-from-entries-to-timing-in-the-database.md) | **The Nightingale roster crosses from `entries` to `timing` in the database, not by a CSV** — `timing.import_from_entries()` joins the two schemas in one transaction, computes age on day in SQL and discards the date of birth in the same expression. *Names what may cross and what may not: `gender_identity`, the phone, both emergency-contact fields and the medical note never do. The deciding argument is that a roster CSV would need a date of birth in it* | 12 Sep 2026 |
| [**ADR-040**](adr-040-a-runners-own-hold-yields.md) | **A runner's own abandoned hold yields to their next attempt** — a submission supersedes a live `pending` hold that names one of its own people, so somebody who reached Stripe and did not pay can enter again at once instead of meeting *"this runner already has a place"* for thirty-one minutes. *Narrows the one-place rules of `20260827090000` and `20260830160000` without widening either: it is keyed on the runner rather than the address, precisely so one submission cannot expire somebody else's in-flight checkout on a shared address* | 12 Sep 2026 |
| [**ADR-041**](adr-041-the-ticket-confirmation-gets-a-skin-of-its-own.md) | **The ticket confirmation gets a skin of its own** — a second renderer, `worker/ticket-email-skin.ts`, rather than a widened `email-skin.ts`, which still knows nothing about a ticket. *The change [ADR-033](adr-033-a-ticket-is-not-an-entry.md) deferred when it called a ticket skin "a separate change". `ticket_refunded` stays text alone, deliberately: no cancellation design was supplied* | 13 Sep 2026 |
| [**ADR-042**](adr-042-publishing-a-result-is-an-act-somebody-takes.md) | **Publishing a result is an act somebody takes, and finishing a race is not it** — `results_published_at` on the event, set by `timing.publish_results()` behind `timing.result.publish`, refused until the race is finished and while anything is on the triage list, and reversible. *Supersedes the mechanism `20260911180000`'s header imagined — publishing by widening who holds `nn.results.read` — which would publish every race at once and could be undone for none of them. Carries the first `anon` grant in `timing`* | 14 Sep 2026 |
| [**ADR-043**](adr-043-a-published-result-carries-a-name-a-category-and-a-time.md) | **A published result carries a name, a category and a time** — `results_for_event()` returns `age_on_day` as null once a race is published, so every caller gets the same answer and `/nn/<year>/results/` can be publicly cached; `result_placement` and `role` join it so the category is `effectiveCategory()`'s answer and a guide is in no band. *Supersedes one consequence of [ADR-042](adr-042-publishing-a-result-is-an-act-somebody-takes.md): `timing.results_published_at()` is a second `anon`-callable function, so a link can ask whether a race is published without pulling the whole field. Whether a band may be published is left to the club* | 14 Sep 2026 |
| [**ADR-044**](adr-044-a-missing-race-under-timing-answers-200.md) | **A missing race under `/timing` answers 200, and a refusal answers 404** — the two statuses stay, one component renders the body in both cases so the wording cannot drift, and all ten tests assert the status as well as the heading. *Declines [#291](https://github.com/southville-running-club/src-website/issues/291)'s own suggestion — the door reading whether the slug names a race — because it costs a second Supabase call on every event page view and a thirty-sixth granted function; the conditions that would change that are written down. Supersedes nothing* | 14 Sep 2026 |
| [**ADR-048**](adr-048-the-club-website-is-its-own-surface.md) | **The club website is its own surface, with its own stylesheet** — Direction A lands in `packages/shared/styles/club.css`, self-contained and importing nothing, with `CLUB_NAV` in a new `club-nav.ts` and new chrome components, so that repainting the club does not repaint the entry form six weeks before the race. *Supersedes nothing: the timing brand adopted on 16 August 2026 still draws `/nn/*`, `/admin/` and `apps/timing`. Records the duplication as a debt with a due date — the post-race change that moves the money pages across* | 19 Sep 2026 |
| [**ADR-049**](adr-049-what-membership-costs-lives-in-the-database.md) | **What membership costs lives in the database** — `membership.membership_types` holds £4 and £27, `membership.settings` holds the minimum age, and `worker/membership.ts` paints all of them onto `/`, `/membership/` and `/membership/join/` at request time, so a fee rise is one `update` and no deploy. *Every failure paints nothing and the page says the price is confirmed on the club's own form. The application form's options ship `hidden disabled` together, because `hidden` alone makes a form unsubmittable. Decides nothing about storing an application: `membership_applications` is still the committee's* | 21 Sep 2026 |
| [**ADR-050**](adr-050-the-club-takes-an-application-and-tells-two-people.md) | **The club takes an application, and tells two people** — `membership.membership_applications` holds the eighteen fields the club already asks on Squarespace, and an outbox owes an acknowledgement to the applicant and the filled form to the Membership Officer. *The committee took the table on 25 September 2026. The two messages carry different amounts about the same person and the **query** enforces that, returning `details: null` for the applicant's copy so the acknowledgement cannot leak a home address. Two keys, because `anon` must be able to call it. Retention starts when a volunteer acts, never when the application arrives. Does not decide the privacy-notice wording, which is why the form is still linked from nowhere* | 25 Sep 2026 |
| [**ADR-051**](adr-051-england-athletics-is-not-a-choice.md) | **England Athletics is not a choice, so the form stops asking** — the club processes every new member on the England Athletics portal and England Athletics sends the payment link, so a yes/no asking permission to share was asking about something that happened either way. *Supersedes ADR-050's consent question and nothing else. The hint beneath it told anybody answering "no" that their details stayed with the club, which was false — worse than not asking. The basis is the contract; the form states it. `ea_portal_consent` is nullable and records null rather than false, because null says "not asked" and false says "refused"* | 26 Sep 2026 |
