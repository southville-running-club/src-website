...
| | | Serves |
| --- | --- | --- |
| [**GitHub setup**](github-setup.md) **①** | People, branch protection, and the **five secrets**. Start here | [Phase 3](../phases.md#phase-3--nightingale-nightmare-live) |
...
| [**Swapping the Stripe keys**](entries-stripe-keys.md) | Test to live and back. **One rule: nothing may be left `paid` across a swap** — a payment can only be refunded by a key of the mode that took it, so the Cancel button refuses it for ever afterwards. Carries the recovery for [#118](https://github.com/southville-running-club/src-website/issues/118)'s stranded payment of 27 August 2026, the per-key scope check, and how to tell a wrong mode from a missing Refunds — Write when both read identically on the page | [Phase 3](../phases.md#phase-3--nightingale-nightmare-live) |
...
| [**The membership keys**](membership-keys.md) | **The two Worker secrets the join form cannot take an application without.** `membership.submit_application()` is granted to `anon` — an applicant is signed out, so it must be — and every accepted call sends two emails, **one to an address the caller chose**. Without a key, a loop makes the club email strangers from its own domain and drains Resend's 100 a day, which is shared with the race. ⚠️ Cloudflare never sees it: a call straight to Supabase is a different origin | [ADR-050](../../architecture/decisions/adr-050-the-club-takes-an-application-and-tells-two-people.md) |
| [**The admin surface**](entries-admin.md) | Roles at `/admin/people/`, the `super-admin` bootstrap, and **what each of the three exports contains** — one of them is special category data. The old two-key scheme (`/nn/admin`, a key per volunteer) is retired; everything under `/admin/` answers 404 to anybody who may not be there | [Phase 3b](../phases.md#phase-3b--member-accounts-before-entries-open) |
| [**Getting into `/timing`**](timing-access.md) | **The cold roster, written down before race morning.** Nobody carries over from the old platform and nobody creates an account for anybody: sign up, then be granted, then be put on the race's roster. Which of the six `timing.*` permissions each role carries, a table of what each role can and cannot open so a refusal can be diagnosed, and **why a granted marshal who is not on the roster gets an ordinary 404** | [Phase 4](../phases.md#phase-4--the-timing-app-on-cloudflare) |
| [**Race night on the timing app**](timing-race-night.md) | **The only end-to-end description of how a race is actually timed**, in six phases from the change freeze to the morning after. Two rules decide the shape of it: **finishing a race is not publishing it** and finishing closes nothing, and a correction after publication is **unpublish, fix, publish** with the public address answering 404 in between — so it is done in one sitting. Carries what a marshal's `queued` card means (nothing) and what the retry cap means (something), and the diagnosis table for "it says Not found" | [Phase 4](../phases.md#phase-4--the-timing-app-on-cloudflare) |
| [**A confirmation nobody received**](entries-email.md) | The outbox, the five-minute cron, and `/admin/emails/` — the queue, the daily-cap figures, and a re-send button. What to check when somebody says they never heard anything, and why "sent today" undercounts the club's real Resend usage | [Phase 3](../phases.md#phase-3--nightingale-nightmare-live) |
...
| [**Selling tickets to a club social**](events-tickets.md) | Putting a social on sale: the three Worker secrets, the second Stripe endpoint, confirming the details and setting the price — in the order that makes opening sales safe. Plus what to check when somebody says they paid and heard nothing, and what the ticket path deliberately cannot do yet | [Phase 5](../phases.md#phase-5--the-new-website) |

> **Retired:** [*Nightingale Nightmare onto the club domain*](nn-to-club-domain.md) — it
> described attaching `nn.southvillerunningclub.co.uk` directly, which
