...
| [**The manual button pass**](manual-test-buttons.md) | **Every control `nn-tester` and `nn-admin` can press**, with a cell to record what happened. The hand-run half of a definition of done: Playwright covers a slice of it, `./dev test` cannot reach Stripe's own pages at all, and neither says anything about the production Worker on the day. **No run is recorded yet** — the document is the list, not the evidence | [Phase 3](../phases.md#phase-3--nightingale-nightmare-live) |
