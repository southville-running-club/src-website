...
| | |
| --- | --- |
| **Time** | Phase 1 an evening. Phase 2 fifteen minutes plus a TTL wait |
...
| **0.3** | `apps/nn` exists in this repository, per [ADR-001](../../architecture/decisions/adr-001-one-monorepo.md) | Monorepo. **Not a separate repository** — [plan](../plan.md) step 17 |
...
Minimum viable. Astro static output, **no adapter** — per
[the build brief](../nn-build-brief.md#the-adapter-constraint--read-this-before-choosing-anything),
and note that for a static-only site Astro needs no Cloudflare adapter at all.
...
| **Build watch paths** | **include `apps/nn/*`** ← so website changes do not rebuild this |
...
**Why watch paths matter.** The free plan allows **500 builds a month**. Without them, every
push to any part of the repository rebuilds this project.

### 1.3 — Verify it actually serves
...
### 1.4 — Confirm a preview deployment appears
