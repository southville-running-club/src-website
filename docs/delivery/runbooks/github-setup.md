...
- [ ] Open a pull request. Eight checks run and pass: **`Documentation`**,
      **`What changed`**, **`Lint, types and tests`** — a real Postgres, migrations applied
      from zero, and the Worker runtime — and five acceptance jobs,
      **`Acceptance (serial)`** plus **`Acceptance (base-1)`** to **`(base-4)`**, which are
      Playwright and axe on a runner each.
      On a documentation-only pull request the last six report as **skipped**, which is
