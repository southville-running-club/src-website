...
| **The race pages** ✅ | **Built**, and now split between the race and one running of it — [ADR-011](../architecture/decisions/adr-011-a-race-and-its-runnings.md). Evergreen at `/nn/`; the confirmed date, the race facts and the entry form at `/nn/2026/`, with `/nn/2026/race-day/` and `/nn/2026/spectators/` beneath it. **`/nn/course/` was the second evergreen page and it is gone** — the club supplied its copy in full as what `/nn/` should carry, so the course and terrain are four sections of that page now and the address 301s there. All of them read from `apps/main/src/content/race.json`. **The first of the race director's confirmed copy landed 24 August 2026** — her opening two paragraphs on `/nn/`, the water station's location, and the guide-places wording in front of the entry form. **The rest of the prose is still a draft pending committee approval.** **And `/nn/2026/` says where the race goes and that Nightingale Valley is the climb, which it never did** — both were on `/nn/course/` alone, so the page carrying the entry form described the ground and the shoes and never the shape of the course somebody was paying to run. The climb reached `/nn/` too, as `NnRaceSummary`'s second bullet. **Both of those facts are stated differently now** — the club supplied the course page's copy in full for `/nn/`, which restates all five bullets at greater length, so that component came off the page and is rendered nowhere at all. **And "one route, one spelling per page" was overruled by the people whose copy it is**: `/nn/` carries the route in the race director's wording and in the club's, and `site.spec.ts` asserts both where it once refused the second |
| **`/nn/2026/` consolidated into one entry point** ✅ | **Built**, and it is one entry *point* rather than one page — [ADR-027](../architecture/decisions/adr-027-one-entry-point-for-a-running.md). The hero, a sticky entry rail, the facts, the race-morning schedule in full, and a summary of each of the two pages below it. `/nn/2026/race-day/` and `/nn/2026/spectators/` both stay live: absorbing them would retire two URLs and no decision record proposes it. **Only the race instructions are still reachable by a link, though** — the hero and the entries-closed notice both point at them, and nothing anywhere points at `/nn/2026/spectators/` since the bar's `Spooktators` item came out, which is the shape `/nn/course/` was in before it was retired with a 301. A page-local jump-nav reaches the three content sections; the masthead beside it carries four hrefs — the wordmark, `Race`, `Privacy` and the painted button — on six of the seven campaign pages, `/nn/2026/entry/complete/` keeping the wordmark alone. **Below 860px this is the one page whose masthead releases**, because it is the one page with a fixed bar at the bottom, and two pinned strips is 35% of a 568px phone. **Nothing has been through a browser yet** — the acceptance layer has not run against any of it |
...
- [x] **How a free place is taken.** ~~Stripe refuses a zero-total Checkout session outright,
      ...
      [ADR-028](../architecture/decisions/adr-028-a-place-can-be-given.md). It is what the two
