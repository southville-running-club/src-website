...
| `accountRetention` | **Settled 31 August 2026.** How long an account is kept. It is a *criterion* rather than a period — nothing deletes an account here, so "until you delete it" is what is true — and the value commits the club to announcing any inactivity clear-out **before** it starts one. Decision 009 |
| `accountDeletionAndEntries` | **Settled 31 August 2026.** What deleting an account does to a race entry: nothing. `identity.delete_me()` takes the `auth.users` row and lets the cascades have the profile and the roles, and `entries` is deliberately not keyed on `identity.people`. ⚠️ **The value also separates the two deletion acts** — the self-service button, and an erasure request, which `/nn/privacy/` section 7 says cancels the entry. Decision 009 |

⚠️ **Both are rendered text on a legal notice, so they are edited the way that page's prose is:
in the same commit as `lastUpdated`, with `privacy.spec.ts` updated alongside.** Emptying either
back to `null` republishes a marker.

`privacy.spec.ts` counts **zero** markers on `/privacy/` — since 31 August 2026 — and
...
directions at once. **Zero is still worth asserting**, and on both pages it now guards the
opposite direction — if one of the four values `/nn/privacy/` interpolates ever goes `null`, or
either of the two answers above does, the page prints "to be confirmed by the club" in the
middle of a published legal notice. **That is the whole reason the constants stay at zero
rather than being deleted with the last marker.**
...
| `ENTRIES_WEBHOOK_KEY` | A **Worker secret**, and the least obvious one. `entries.record_checkout_event()` is granted to `anon` like every other function, and the anon key is published in page source — so the key is what stops two PostgREST calls buying a free entry. `entries.webhook_secrets` holds only its SHA-256 digest — ⚠️ **the table at rest, not the path: it travels as the function's first argument, so it is in an RPC body on every delivery.** Issue [#21](https://github.com/southville-running-club/src-website/issues/21); Postgres statement logging was checked clean on 30 August 2026, the API request logs are **unverified**, and [the attention runbook](../../../docs/delivery/runbooks/entries-attention.md#the-webhook-key-travels-as-an-rpc-argument--one-check-still-owed) owns that check and the rotation — **step 3 below** — it would call for |
...
**Tickets to the club's socials add three more — steps 17 to 19 — and none of them is shared with
the race.** `store` holds its own key digests in `store.api_secrets`, so a compromise of the party
ticket path is not a compromise of the race payment path, and a Stripe signing secret is per
endpoint regardless. ⚠️ **Step 17 must be verified before `store.socials.sales_open_at` is set**;
the other order is a ticket window that is open and unprotected. See
[ADR-033](../../../docs/architecture/decisions/adr-033-a-ticket-is-not-an-entry.md) and
[the runbook](../../../docs/delivery/runbooks/events-tickets.md). They are listed below as pending because nothing has been set on the deployed
...
| _5. Create the Stripe webhook endpoint_ | **Last, and only once the Worker is deployed.** Otherwise Stripe posts into a 404 | **Done** — inferred rather than recorded: payments are being confirmed, which nothing but this endpoint can do. Confirmed 7 Sep 2026 | Stripe dashboard → Developers → Webhooks → Add endpoint. URL `https://new.southvillerunningclub.co.uk/nn/stripe-webhook`. Subscribe to **`checkout.session.completed` and `checkout.session.expired` and nothing else** — everything else is answered 200 and ignored, and subscribing to more is delivery volume for no benefit |
...
| _15. Swap Stripe to live keys_ | **The last thing before entries open, and it is deliberately not step 2.** Test mode proves the integration; it does not prove the club's live account, its payouts, or that a live-mode webhook endpoint exists — those are separate objects in Stripe from the test ones | **Done**, confirmed 7 Sep 2026 — the club is taking real payments for NN 2026 | [Entries-open step 2c](../../../docs/delivery/runbooks/entries-open.md#2c--then-live-mode-for-a-pound). Register the **live-mode** endpoint, then `wrangler secret put` both `STRIPE_SECRET_KEY` and `STRIPE_WEBHOOK_SECRET` with the live pair, then buy one £1 **Tester** entry with a real card and cancel it |
| _16. Set the entry key, and install its digest_ | **The same two-step shape as step 3, one function along, and the one that was missing.** `entries.create_pending_purchase()` is granted to `anon` and holds a place before any money moves; without this key anybody holding the published anon key takes all 250 places in half a second, for nothing, without touching this Worker or Cloudflare's rate-limiting rule. Issue #178, [ADR-029](../../../docs/architecture/decisions/adr-029-holding-a-place-takes-a-key.md) | **Done** — inferred rather than recorded: it was a stop condition for opening the entry window, and the window is open with 110 of 250 places sold, so it cannot not be. Confirmed 7 Sep 2026 | ⚠️ **Before `entries_open_at` is set, never after.** `openssl rand -base64 32`, then `npx wrangler secret put ENTRIES_ENTRY_KEY --env production --config apps/main/wrangler.jsonc`, then in the Supabase SQL editor: `update entries.webhook_secrets set key_sha256 = encode(sha256(convert_to('<the key>','UTF8')),'hex'), updated_at = now() where name = 'entry';`. Verify with one `nn-tester` entry — full procedure and the two failure signatures are [entries-open step 0.8](../../../docs/delivery/runbooks/entries-open.md#08--the-entry-key-must-be-installed-and-verified) |
| _17. Set `STORE_ENTRY_KEY`, and install its digest_ | **The same two-step shape as steps 3 and 16, one schema along.** `store.create_pending_purchase()` is granted to `anon` — a signed-out buyer reaches PostgREST as `anon` — and it holds tickets *before* any money moves, so without a second factor a loop with the published anon key takes the whole party for nothing. **A separate key from `ENTRIES_ENTRY_KEY` on purpose**: one key opening two doors is one rotation closing both. ⚠️ **This must be done and verified before `store.socials.sales_open_at` is ever set** — the other order is a ticket window that is open and unprotected | **Pending** | `npx wrangler secret put STORE_ENTRY_KEY --env production --config apps/main/wrangler.jsonc` from `platform/`, then `update store.api_secrets set key_sha256 = '<sha256>' where name = 'entry'`. [The runbook](../../../docs/delivery/runbooks/events-tickets.md#01--the-three-worker-secrets) |
| _18. Set `STORE_WEBHOOK_KEY`, and install its digest_ | The second factor `store.record_checkout_event()` demands, for the same reason step 3 exists on the race path: the function that writes `paid` is anon-callable and the anon key is published in page source. It also guards the two outbox drain functions, which hand back real email addresses | **Pending** | `npx wrangler secret put STORE_WEBHOOK_KEY --env production --config apps/main/wrangler.jsonc` from `platform/`, then `update store.api_secrets set key_sha256 = '<sha256>' where name = 'stripe'` |
| _19. Create the second Stripe webhook endpoint, and set `STORE_STRIPE_WEBHOOK_SECRET`_ | **A second endpoint, not a second subscription on the race's.** A Stripe signing secret is per endpoint, so `/events/stripe-webhook` cannot share `/nn/stripe-webhook`'s — and sharing one would mean a rotation for the shop closing the race's payments. Needs the production URL, so it is a human's job exactly as step 5 is | **Pending** | ⚠️ **In live mode.** The club's Stripe is on live keys since NN 2026 entries opened, and a test-mode endpoint is a different object with a different signing secret — one created in the wrong mode never fires, and the symptom is tickets that are paid for in Stripe and never confirmed on the site. Add an endpoint at `https://new.southvillerunningclub.co.uk/events/stripe-webhook` subscribed to `checkout.session.completed` only, then `npx wrangler secret put STORE_STRIPE_WEBHOOK_SECRET --env production --config apps/main/wrangler.jsonc` from `platform/`. [The runbook](../../../docs/delivery/runbooks/events-tickets.md#03--the-stripe-endpoint) |
