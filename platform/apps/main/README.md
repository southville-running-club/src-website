...
| `ENTRIES_ADMIN_KEY` | **Retired, and it should not be bound at all — see the note below the manual-steps table.** #58 retired the two-key admin scheme in the Worker; the admin surface is now reached at `/admin/` by signing in and holding a role (`nn-admin`, `people-admin` or `super-admin`), checked through `identity.has_permission()`. `/nn/admin/*` only 301/308-redirects to `/admin/` now — this key opens nothing |
