 ...
 *
 * Three parts, deliberately, rather than one sentence with tags inside it: Astro compresses
 * the newline between a tag and the text after it to *nothing*, so the mixed form silently
 * renders as "...soon.For everything else...". They are flex items and the gap does the
 * spacing, which is also what lets each part drop onto its own line on a phone.
 */
export const SITE_BANNER = {
  ...
  /**
   * Short deliberately: with the hostname spelled out this wrapped to two lines on a 320px
   * phone and the banner ate a third of the first screen. The wording still says where it
   * goes when a screen reader reads it out of a link list, which "click here" never does.
   */
  clubWebsiteLabel: 'For everything else, please see the old site',
} as const;
