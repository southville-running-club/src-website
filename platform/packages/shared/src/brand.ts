...
/**
 * The club wordmark, traced to vector.
 *
 * **Replaced on 16 August 2026** with the "SRC" monogram, extracted path-for-path from
 * `logo_src.pdf` — vector source supplied directly, not a hand trace of a rendered image —
 * rather than the previous full "SOUTHVILLE RUNNING CLUB" two-line lockup. That artwork is
 * still exactly right for a banner; it read as an illegible smear wherever it was shrunk
 * into a small space (the favicon most of all — see `apps/main/public/favicon.svg`'s
 * history in `docs/foundations/brand.md`), and the club supplied real "SRC" artwork rather
 * than this repository inventing one, which is what the monogram bullet in that document
 * had been waiting on.
 *
 ...
 * somebody has already shared — and `packages/shared/tests/brand.test.ts` asserts the two
 * agree path for path. A logo that disagrees with itself across a site is the exact failure
 * this whole change exists to end.
 *
 * The viewBox is `logo_src.pdf`'s own page size — 412.236 x 215.679 — extracted rather than
 * chosen.
 */
export const CLUB_LOGO = {
  viewBox: '0 0 412.236 215.679',
  /** The accessible name. The club's name, even though the mark itself now reads "SRC". */
  title: 'Southville Running Club',
  /**
   * Three paths — S, R, C — each its own closed contour (the R's bowl is a second,
   * counter-wound subpath within its `d`, for the hole). Filled with `currentColor` by the
