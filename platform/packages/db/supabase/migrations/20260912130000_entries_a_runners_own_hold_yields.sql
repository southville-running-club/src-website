-- ===========================================================================================
-- A runner's own abandoned hold stops standing in their way
-- ===========================================================================================
-- **The report was "their entry says Hold expired and they are not allowed to buy a new one",
-- and half of that sentence describes a state that has never blocked anything.** Both one-place
-- rules in `create_pending_purchase()` count a place as `paid`, or `pending` with a hold that
-- has not lapsed. `expired` matches neither, so an expired hold has always let somebody try
-- again and `entries-rules.test.ts` has always said so.
--
-- **What actually refused them is the 31 minutes before that.** Somebody fills the entry form
-- in, reaches Stripe and does not pay — the tab is closed, the card is declined, the phone
-- rings. Their hold stays `pending` and live, and a second attempt in that window is refused
-- `already_entered` by the name rule and `email_already_entered` by the address rule. The form
-- then says *"This runner already has a place in this race. The club has an entry recorded
-- against this name and date of birth"* and points at `/account/entries/`, where there is
-- nothing — a sentence that is false, about a record that does not exist, with no way forward
-- on the page.
--
-- **The two halves reach a volunteer separately, which is what made this read as a permanent
-- lock-out rather than as a five-minute one.** By the time anybody looks at `/admin/nn/`, the
-- cron has swept the hold and the row says "Hold expired" — so the refusal is remembered
-- against a status that was not the one doing the refusing.
--
-- **A live hold is deliberately counted, and that reasoning is not being thrown away.**
-- `hasConfirmedEntry()` in `apps/main/worker/nn-entry.ts` states it: *"the database rule is
-- broader — it counts a live hold too, because two simultaneous submissions must not both
-- succeed"*. That is right, and it stays right. What was missing is the distinction between two
-- submissions from two people and two submissions from **one** person: the first is a race the
-- rule exists to settle, and the second is somebody trying again.
--
-- So a submission now **supersedes** a live hold that names one of its own people: the hold is
-- expired, its place and any discount use go back, and the entry proceeds. One runner still
-- cannot hold two places at once — the old hold is dead before the new one is written — so the
-- rule's purpose survives intact and only its casualty changes.
--
-- ⚠️ **Keyed on the address *and* the runner together, and each half alone was tried and
-- rejected by a failing test rather than by argument.** The address alone lets two partners on
-- one shared card collide — the second submission expires the first one's hold while they are on
-- Stripe's page, and they then pay for a place that has gone back. The runner alone does the
-- same damage from further away: a **different** purchaser entering with this person as their
-- **guide** names them on their own entry, so `entries-guides.test.ts`'s "refuses a guide who
-- already holds a place of their own" went green-to-red as an accepted duplicate. Requiring both
-- means only the person coming back to their own abandoned checkout supersedes anything.
--
-- **Nothing that was already asserted changes, and that is the measure of how narrow this is.**
-- The 30 August 2026 decision that one address holds one place stands untouched — a *second*
-- runner on an address that already holds a place is refused exactly as before — and so does the
-- name rule's independence from the card. No existing test needed its fixtures changed.
--
-- **What it narrows, it narrows deliberately:** a runner who abandoned a checkout and comes back
-- typing a *different* address still waits for the hold to lapse. A signed-in runner cannot
-- reach that case at all, because their address comes from their session and not from the form.
--
-- **`create_manual_entry()` and `transfer_entry()` are not changed.** Both are a volunteer
-- acting on one record at a time with the person in front of them; neither is somebody stuck on
-- a form at midnight, which is the whole case this is for. A volunteer who meets
-- `already_entered` while giving a place away can see the live hold on `/admin/nn/` and wait
-- half an hour — and superseding a hold from the admin surface without saying so is the kind of
-- silent write ADR-024 and ADR-028 were both careful not to add.
--
-- **One latent defect is closed alongside it, because the fix touches the same predicate.**
-- `entries.entry_purchases` has never had the constraint `store.ticket_purchases` was given on
-- its first day: a `pending` row with a null `hold_expires_at` would be read as a hold that
-- never lapses — blocking that runner and that address for the rest of the year — and
-- `expire_pending_holds()`, which only expires holds that have a date, could never clear it.
-- Nothing writes one today: `create_pending_purchase()` always sets 31 minutes and
-- `create_manual_entry()` inserts `paid`. The guard is so that it stays that way.
--
-- See ADR-040, which records the decision and what it costs.


-- -------------------------------------------------------------------------------------------
-- A pending purchase holds a place, so it has to say when the place goes back
-- -------------------------------------------------------------------------------------------
-- **`NOT VALID`, per this repository's rule for a new check constraint.** It protects every new
-- write immediately and does not ask about rows already in production, which nobody here can
-- see — a validated `add constraint` would fail the whole migration on one disagreeing row and
-- take every migration behind it down with it. Validating it is
-- `docs/delivery/runbooks/entries-constraints.md`, which owns the other four.
--
-- Same rule and same words as `store.ticket_purchases`' `ticket_purchases_hold_when_pending`,
-- written on that table's first day — this one is five schemas' worth of hindsight later.
alter table entries.entry_purchases
  add constraint entry_purchases_hold_when_pending
  check (status <> 'pending' or hold_expires_at is not null)
  not valid;


-- -------------------------------------------------------------------------------------------
-- entries.create_pending_purchase() — a runner's own live hold is replaced, not counted
-- -------------------------------------------------------------------------------------------
-- Re-pasted, not patched, from 20260831170000_entries_results_placement.sql — byte-for-byte
-- apart from this header, one declaration, the block that computes `v_superseded`, the three
-- predicates that exclude it and the block that hands the places back.
--
-- **The signature is unchanged**, so this replaces the function rather than creating a second
-- overload beside it, and the grants and the anon list are untouched —
-- `packages/db/tests/entries.test.ts` still asserts the same thirteen names and the same seven
-- key-takers.
...
declare
  ...
  -- **The live holds this submission replaces.** Empty for almost every entry; one row for
  -- the runner who reached Stripe, did not pay, and has come back. Computed under the
  -- capacity lock, read by three predicates below and written only outside a preview.
  v_superseded uuid[];
begin
  ...
  -- --- a hold this runner left behind is replaced, not held against them ---------------------
  -- **The defect this closes is the entry form's most ordinary failure.** Somebody fills the
  -- form in, reaches Stripe and does not pay: they close the tab, the card is declined, the
  -- phone rings. Their `pending` hold then lives for 31 minutes, and for those 31 minutes both
  -- one-place rules below counted it as a place — the name rule refused them `already_entered`
  -- and the address rule refused them `email_already_entered`. The form told them *"This runner
  -- already has a place in this race"*, which was false: nothing was charged, no place was
  -- confirmed, and the page's only advice pointed at an entries list with nothing on it.
  --
  -- **The two halves of that arrive separately, which is what made it read as permanent.** By
  -- the time a volunteer looks, the five-minute sweep has run and `/admin/nn/` says "Hold
  -- expired" — so the report is *"their entry says Hold expired and they are not allowed to buy
  -- a new one"*, describing a state that never blocked anything alongside a refusal that came
  -- from a state that no longer exists. See ADR-040.
  --
  -- **Both identities have to agree, and each one alone is a way to destroy somebody else's
  -- in-flight checkout.** A submission supersedes a live hold when the hold was bought by the
  -- **same address** *and* somebody on this submission **is** somebody on that hold — first
  -- name, last name and date of birth, compared exactly the way the rule below compares them.
  --
  -- ⚠️ **Neither half is belt-and-braces; both were written after watching the other half fail
  -- on its own.**
  --
  --   * **The address alone** would let two partners on one shared card collide: the second one
  --     to submit would expire the first one's hold while they were still on Stripe's page, and
  --     the first would then pay for a place that had gone back. The address rule of 30 August
  --     2026 is what handles that pair, and it handles it by refusing the second — which it
  --     still does, because a different runner never supersedes anything.
  --   * **The runner alone** does the same damage one step further out, and
  --     `entries-guides.test.ts` is what caught it: a *different* purchaser entering with this
  --     person as their **guide** names them on their own entry, so a runner who already held a
  --     live place of their own would have had it expired by a stranger's submission. That is
  --     also a rule — a guide may not hold an entry of their own — being silently unenforced
  --     rather than answered.
  --
  -- What this narrows in exchange is one case: a runner who abandoned a checkout and comes back
  -- typing a **different** address still meets the rules below until their hold lapses. That is
  -- the same residual the mistyped-name case leaves, it is what the message on the page is for,
  -- and a signed-in runner cannot reach it at all — their address comes from their session.
  --
  -- **`pending` only.** A `paid` place is a place and still refuses them, which is the rules'
  -- entire purpose; `refunded` and already-`expired` rows never blocked anybody. A hold whose
  -- 31 minutes have already run out is matched even though it does not block, because handing
  -- its place and its discount use back now rather than at the next sweep costs nothing — and a
  -- `pending` row with a null `hold_expires_at`, which `expire_pending_holds()` can never reach
  -- at all, is matched for the same reason. `entry_purchases_hold_when_pending` above is what
  -- stops another one being written.
  --
  -- ⚠️ **A superseded hold may already have a Stripe Checkout session attached to it, and
  -- paying that later is the same late payment a hold that simply lapsed can take.** `paid`
  -- still wins — the webhook never refuses money — and the place is flagged `over_capacity` if
  -- there is no room for it, which is the queue a volunteer already watches daily. This makes
  -- that window arrive sooner; it does not create it, and it is the accepted cost of the
  -- alternative being a runner who cannot enter at all.
  --
  -- **Computed in a preview too, and written only outside one.** A preview has to answer what a
  -- real submission a moment later would answer, so it excludes the same holds from the same
  -- three places; what it does not do is expire them. Somebody who reads a total and closes the
  -- tab has still cost nobody anything.
  --
  -- ⚠️ **A date that does not parse supersedes nothing, and this block is not decoration.**
  -- The minimum-age loop further down is the first place that casts `date_of_birth`, and it
  -- carries a guard of its own so a payload sending `'banana'` is *answered*
  -- `invalid_entrants` rather than raising. This runs earlier, so without the same guard that
  -- submission would blow out of here as a Postgres error instead — a caller that has been
  -- told "there is something wrong with the entrant block" since Slice G would start getting
  -- `invalid_datetime_format` from PostgREST, which the Worker renders as "your entry could
  -- not be completed" and logs as drift. Falling through with an empty set leaves every
  -- refusal on this path answering precisely what it answered before.
  begin
    select coalesce(pg_catalog.array_agg(distinct purchase.id), array[]::uuid[])
      into v_superseded
      from entries.entry_purchases as purchase
      join entries.entrants as entrant
        on entrant.purchase_id = purchase.id
      join pg_catalog.jsonb_array_elements(p_entrants) as submitted(value)
        on pg_catalog.lower(pg_catalog.btrim(entrant.first_name))
           = pg_catalog.lower(pg_catalog.btrim(submitted.value ->> 'first_name'))
       and pg_catalog.lower(pg_catalog.btrim(entrant.last_name))
           = pg_catalog.lower(pg_catalog.btrim(submitted.value ->> 'last_name'))
       and entrant.date_of_birth = (submitted.value ->> 'date_of_birth')::date
     where purchase.event_id = v_event.id
       and purchase.status = 'pending'
       -- **`lower(…::text)` and never the `citext` `=` this column's type invites**, for the
       -- reason spelled out in full at the address rule below: this function runs
       -- `set search_path = ''`, the `citext` equality operator lives in `extensions`, and
       -- Postgres silently resolves the comparison to plain text instead of raising. Getting
       -- it wrong here fails in the harmless direction — one capital letter and the hold is
       -- simply not superseded — which is exactly why it would never have been noticed.
  ...
  select pg_catalog.count(*)::int
   ...
   where purchase.event_id = v_event.id
     -- **A hold this submission replaces is not a place somebody else has taken, and leaving
     -- it in this count is the half that would have looked like a fix without being one.** On
     -- a race with one place left, a runner coming back to an abandoned checkout would be
     -- told `sold_out` by their own dead hold — the same person locked out by the same row,
     -- with a different word on the page.
     and not (purchase.id = any(v_superseded))
     and (
    ...
    if exists (
      select 1
       ...
       where purchase.event_id = v_event.id
         ...
         -- Their own unfinished attempt is not an entry they hold. Every live hold naming
         -- one of these people is in `v_superseded` by construction, so after this exclusion
         -- the rule fires on a **`paid`** place and on nothing else — which is exactly what
         -- the notice it produces claims.
         and not (purchase.id = any(v_superseded))
         and (
           purchase.status = 'paid'
           or (
  ...
  if exists (
    select 1
     ...
     where purchase.event_id = v_event.id
       and pg_catalog.lower(purchase.purchaser_email::text)
           = pg_catalog.lower(pg_catalog.btrim(p_purchaser_email))
       -- **The same exclusion, and without it the fix above does nothing for anybody.** A
       -- runner's abandoned hold carries their address as well as their name, so excluding it
       -- from the rule above and leaving it here would refuse them at the next line down — on
       -- a rule about the address, with a message telling them to use a different one.
       --
       -- ⚠️ **This does not widen the rule itself.** Only holds naming one of *this*
       -- submission's people are in `v_superseded`, so a different runner on an address that
       -- already holds a place is refused exactly as they were on 30 August 2026. That trade
       -- is still in force and still costs the couple entering on one card.
       and not (purchase.id = any(v_superseded))
       and (
         purchase.status = 'paid'
         or (
  ...
  begin
    -- **The replaced holds go back before anything new is written.** `status = 'expired'` is
    -- the state `expire_pending_holds()` writes and means the same thing here — the place went
    -- back — so `/admin/nn/` reads "Hold expired" for a replaced hold exactly as it does for
    -- one that timed out. The two are deliberately indistinguishable on that page: both say no
    -- money was taken and no place is held, which is the whole of what a volunteer needs from a
    -- row with no runner coming.
    --
    -- **A discount use goes back with the place**, for the reason the sweep returns it: a use
    -- that is only ever spent would let a handful of abandoned checkouts exhaust the whole
    -- allocation with nobody entered. A submission re-using the same code decrements here and
    -- increments below, which is a net nothing and is correct.
    --
    -- **Inside this block, so the tables refusing what comes next rolls it back with
    -- everything else.** Somebody whose new entry turns out to be invalid must not be left
    -- having lost the hold they already had.
    --
    -- `greatest` bare rather than `pg_catalog.greatest`: it is a SQL construct rather than a
    -- function, so a schema qualification does not resolve. `expire_pending_holds()` writes
    -- the same line the same way.
    if pg_catalog.array_length(v_superseded, 1) is not null then
      update entries.discount_codes as code
         set uses = greatest(0, code.uses - returned.given_back)
        from (
          select superseded.discount_code_id,
                 pg_catalog.count(*)::int as given_back
            from entries.entry_purchases as superseded
           where superseded.id = any(v_superseded)
             and superseded.discount_code_id is not null
           group by superseded.discount_code_id
        ) as returned
...
comment on function entries.create_pending_purchase(
  ...
  'Price one entry, hold a place for it and record a pending purchase. Takes the entry key: granted to anon because a signed-out runner reaches PostgREST as anon, and refusing without the key is what stops that grant from being 250 free places to anybody holding the published key. Refuses a total of zero — Stripe cannot take one, so the place could never be completed. A live pending hold naming one of this submission''s own people is superseded rather than counted — expired, with its place and any discount use returned — so a runner who abandoned a checkout can enter again at once; a paid place still refuses them, and an address that already holds a place still refuses a different runner. See ADR-029, ADR-040 and issue #178.';
