...
| _Add `RESEND_SMTP_PASSWORD`_ | `deploy-db.yml`'s `supabase config push` sends `config.toml`'s `[auth.email.smtp]` block, which reads this via `env(...)` — #50. **A different secret from the Worker's `RESEND_API_KEY`**, even though both are the same Resend account, because a deploy credential and a client-reachable Worker credential should not be interchangeable | _pending_ | Repository → Settings → Secrets and variables → Actions. Value is a Resend API key — the same key already set as the Worker's `RESEND_API_KEY`, or a second key with the same Sending-access-only scope if you'd rather not reuse one credential for two purposes |
