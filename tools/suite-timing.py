...
def main():
    # **Every report given, merged into one table.** The suite is two Playwright configs run
    # separately, and a summary of either alone describes half the work while looking like all
    # of it. A report that cannot be read is skipped rather than fatal: one config crashing
    # hard enough not to write its JSON is a reason to want the other config's numbers, not a
    # reason to lose them too. The shell glob passes nothing when no report exists at all.
    if len(sys.argv) < 2:
        return 0
    ...
    for path in sys.argv[1:]:
        try:
            ...
            unreadable.append(f"{path.split('/')[-1]}: {exc}")
            continue
        for suite in report.get("suites", []):
            rows.extend(walk(suite))

    for note in unreadable:
        print(f"_No timing summary for {note}_\n")
